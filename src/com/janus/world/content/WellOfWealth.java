...
public class WellOfWealth {
    ...
    public static boolean checkFull(Player player) {
        if (STATE == WellState.FULL) {
            DialogueManager.start(player, new Dialogue() {
                ...
                @Override
                public String[] dialogue() {
                    return new String[]{"The fountain is already full of coins and Saradomin", "has granted players with x2 drops for their", "generosity! There are currently " + getMinutesRemaining() + " minutes", "of x2 Drops left."};
                }
    ...
    public static void donate(Player player, int amount) {
        ...
        if (getMissingAmount() <= 0) {
            ...
            World.sendMessage("@red@[Fountain]@bla@Is now granting everyone 1 hour of x2 Drop Rates");
        }
    }
    ...
    public static void updateState() {
        if (STATE == WellState.FULL) {
            if (getMinutesRemaining() <= 0) {
                World.sendMessage("<img=10> <col=6666FF>The Fountain of Goodwill is no longer granting x2 Drop Rates");
