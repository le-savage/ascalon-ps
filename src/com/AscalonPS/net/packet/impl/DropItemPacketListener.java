        ...
        if (item != null && item.getId() != -1 && item.getAmount() >= 1) {
            if (item.tradeable() && !ItemBinding.isBoundItem(item.getId())) {
                ...
                if (item.getId() == 4045) {
                    ...
                    PlayerLogs.log(player.getUsername(), "Player dropped item: " + item.getId() + ", amount: " + item.getAmount());
                    //	DiscordMessenger.sendDropLog(player.getUsername() + " dropped : "+item.getDefinition().getName()+"     ID:   " +item.getId()+"     AMOUNT:   "+item.getAmount()+"     VALUE:    "+item.getDefinition().getValue()*item.getAmount()+"     ["+player.getPosition().getX()+" Y: "+player.getPosition().getY()+" Z: "+player.getPosition().getZ()+"]");
                }
