...
import com.AscalonPS.model.GameMode;
...
public class Trading {
    ...
    public void giveItems() {
        ...
        try {
            ...
            player2.getPacketSender().sendRichPresenceState("Exploring Janus..");
            player2.getPacketSender().sendRichPresenceSmallPictureText("CB LVL: " + player.getSkillManager().getCombatLevel());
            player2.getPacketSender().sendSmallImageKey("home");
            ...
            for (Item item : player.getTrading().offeredItems) {
                String tradeSameIP = (player.getUsername().toUpperCase() + " [GAVE: " + item.getDefinition().getName() + "] [ID: " + item.getId() + "] [Amount: " + item.getAmount() + "] [Value: " + item.getDefinition().getValue() + "] [TO: " + player2.getUsername() + "] [SAME IP TRADE]");

                String tradeInfo = (player.getUsername().toUpperCase() + " [GAVE: " + item.getDefinition().getName() + "] [ID: " + item.getId() + "] [Amount: " + item.getAmount() + "] [Value: " + item.getDefinition().getValue() + "] [TO: " + player2.getUsername() + "] [IP SENDER: " + player.getHostAddress() + "] [IP RECEIVER: " + player.getHostAddress() + "]");
			...
			/*for (Item item : player2.getTrading().offeredItems) {
				...
				//PlayerLogs.logTrade(player2.getUsername().toUpperCase(), player2.getUsername().toUpperCase()+" "+tradeInfo);
			}*/
            ...
            trade.update("b2U8iPavtVJqaK1JhjNetXSAGwNKOx7EvOJWQTUjOhQo771o5uUUxRjPce47qm5cTbTqqw0p");
    ...
    public void requestTrade(Player player2) {
        ...
        if ((player2.getGameMode() == GameMode.IRONMAN) && player.getRights() != PlayerRights.OWNER) {
            player.getPacketSender().sendMessage("That player is a Hardcore-ironman-player and can therefore not stake.");
            return;
        }
        if ((player2.getGameMode() == GameMode.HARDCORE_IRONMAN) && player.getRights() != PlayerRights.OWNER) {
        ...
        if (player.getUsername().equals(player2.getUsername())) {
            player.getPacketSender().sendMessage("You shouldn't be able to do this! Logs sent to staff!");
            //DiscordMessenger.sendTradeLog("[SAME NAME TRADE] "+player.getUsername().toUpperCase()+ " TRADED " +player2.getUsername().toUpperCase());
            return;
        }
	...
	/*	if(player.getNewPlayerDelay() > 0 && player.getRights().ordinal() == 0) {
        ...
        if (player.getRights() != PlayerRights.OWNER && player2.getRights() != PlayerRights.OWNER && !(itemId == 1419 && player.getRights().isStaff())) {
