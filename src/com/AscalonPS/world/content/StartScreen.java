...
public class StartScreen {
    ...
    public static boolean handleButton(Player player, int buttonId) {
        ...
        String newPlayerMsg = ("<col=6600CC>[New Player]: " + player.getUsername() + " has logged into Janus for the first time!");
        if (buttonId == CONFIRM) {
            ...
            if (!PlayerPunishment.hasRecieved1stStarter(player.getHostAddress())) {
                ...
                World.sendMessage(newPlayerMsg);
                ...
                World.sendMessage(newPlayerMsg);
                player.getPacketSender().sendInterface(3200);
                //player.getDailyReward().openInterface();//DAILY REWARD 2ND STARTER ACC
            } else if (PlayerPunishment.hasRecieved1stStarter(player.getHostAddress()) && PlayerPunishment.hasRecieved2ndStarter(player.getHostAddress())) {
                //player.getPacketSender().sendInterfaceRemoval();
                ClanChatManager.join(player, "help");
                player.setPlayerLocked(false);
