...
import com.AscalonPS.engine.task.impl.CombatSkullEffect;
...
import com.AscalonPS.model.movement.PathFinder;
...
import com.AscalonPS.world.content.BonusManager;
import com.AscalonPS.world.content.ItemDegrading;
import com.AscalonPS.world.content.ItemDegrading.DegradingItem;
import com.AscalonPS.world.content.Kraken;
import com.AscalonPS.world.content.combat.effect.CombatPoisonEffect;
import com.AscalonPS.world.content.combat.effect.CombatPoisonEffect.PoisonType;
import com.AscalonPS.world.content.combat.magic.CombatAncientSpell;
...
import com.AscalonPS.world.entity.impl.npc.NPCMovementCoordinator.CoordinateState;
...
public final class CombatFactory {
    ...
    public static void poisonEntity(Character entity, Optional<PoisonType> poisonType) {
        ...
        if (entity.isPlayer()) {
            ...
            if ((player.getPoisonImmunity() > 0) || (player.getEquipment().getItems()[Equipment.HEAD_SLOT].getId() == 12282) || (player.getEquipment().getItems()[Equipment.HEAD_SLOT].getId() == 12278) || (player.getEquipment().getItems()[Equipment.HEAD_SLOT].getId() == 12279))
                return;
    ...
    public static int calculateMaxRangedHit(Character entity, Character victim) {
        ...
        if (victim != null && victim.isNpc()) {
            ...
            if (npc.getId() == player.getSlayer().getSlayerTask().getNpcId()) {
                if (player.getEquipment().getItems()[Equipment.CAPE_SLOT].getId() == 21020) {
                    maxHit *= 1.30;
                }
            }

            if (npc.getDefinition().getName().contains("dragon")) {
                if (player.getEquipment().getItems()[Equipment.WEAPON_SLOT].getId() == 4453) {
                    maxHit *= 1.20;
                }
            }

            if (npc.getDefinition().getName().contains("dragon")) {
                if (player.getEquipment().getItems()[Equipment.WEAPON_SLOT].getId() == 21051) {
                    maxHit *= 1.20;
                }
            }
    ...
    public static boolean checkHook(Character entity, Character victim) {
        ...
        if (victim.isNpc() && entity.isPlayer()) {
            ...
            if (npc.getId() == 2891) {
                for (int i = 0; i < 4; i++) {
                    if (!((Kraken.KrakenInstance) entity.getRegionInstance()).disturbedPool(i)) {
