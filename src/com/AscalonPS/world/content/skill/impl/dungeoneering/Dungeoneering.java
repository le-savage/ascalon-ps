package com.AscalonPS.world.content.skill.impl.dungeoneering;
    ...
    public static void start(final Player p) {
        ...
        for (Player member : party.getPlayers()) {
            if (member != null) {
                ...
                member.getPacketSender().sendRichPresenceState("Dungeoneering!");
                member.getPacketSender().sendSmallImageKey("attack");
                member.getPacketSender().sendRichPresenceSmallPictureText("Lvl: " + member.getSkillManager().getCurrentLevel(Skill.DUNGEONEERING));
